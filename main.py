...
sklearn2pmml(make_pmml_pipeline(pipeline), "svm_pmml.pmml")
...
#
# print("Prediction",model.predict(["i hate swimming"]))
#
# svm_model = "svm.pkl"
# joblib.dump(model, svm_model)


# tensorflow.keras.models.save_model(model, 'svm.pkl')
# converter = tensorflow.lite.TFLiteConverter.from_keras_model(model=model)
# model_tflite = converter.convert()
# open("SupportVectModel.tflite", "wb").write(model_tflite)
...
# obj = joblib.load('svm.pkl')
# pmml_pipeline = make_pmml_pipeline(obj)
# sklearn2pmml(pmml_pipeline, 'model.pmml')

# pipeline = PMMLPipeline([('vect', TfidfVectorizer(ngram_range=(1, 2), stop_words="english", sublinear_tf=True)),
#                      ('chi', SelectKBest(chi2, k=10000)),
#                      ('clf', svm.SVC(C=2.0, kernel='linear', gamma='auto'))])
...
#
# pipeline = PMMLPipeline([('transformer', vectorizer), ('clf', svm.SVC(C=2.0, kernel='linear', gamma='auto'))])
